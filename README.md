...
This repository is a public implementation of the paper-core BaSPR workflow:

- proteome normalization
- initial key-protein candidate selection
- iterative sparse pruning to a sparse-protein panel
- BaSPR reconstruction training and ablation benchmarking
...
3. Train the BaSPR reconstruction model:
...
python scripts/run_benchmark_reconstruction.py \
  --manifest path/to/data_manifest.yaml \
  --checkpoint work/reconstruction/baspr_checkpoint.pt \
...
python scripts/run_age_benchmark.py \
  --manifest path/to/data_manifest.yaml \
  --checkpoint work/reconstruction/baspr_checkpoint.pt \
  --generated-manifest work/benchmark_reconstruction/baspr/baspr_generated_manifest.json \
