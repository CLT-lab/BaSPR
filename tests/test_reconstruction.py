...
from baspr.reconstruction.infer import reconstruct_pairs
...
def test_reconstruction_training_and_inference_modes(tmp_path, observed_tables_full) -> None:
    ...
    baspr_pair, _ = reconstruct_pairs(
    ...
    baspr_df = baspr_pair["2012_2020"]
    baseline_df = baseline_pair["2012_2020"]
    key_df = key_pair["2012_2020"]

    assert baspr_df.shape[0] == observed_tables_full[2020].shape[0]
    assert baspr_df.shape[1] == observed_tables_full[2020].shape[1]
    assert not np.allclose(
        baspr_df[feature_columns].to_numpy(dtype=float),
