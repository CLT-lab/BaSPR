...
from baspr.reconstruction.dataset import ReconstructionDataset, build_pair_arrays, split_indices_by_id
from baspr.reconstruction.model import BaSPRReconstructionModel
from baspr.reconstruction.train import apply_mode_to_batch, reconstruction_loss, set_global_seed
...
def _train_one_round(
    ...
    *,
    ...
    model = BaSPRReconstructionModel(
    ...
    for _ in range(max(1, epochs)):
        ...
        for baseline, key, target, _ in train_loader:
            ...
            baseline, key = apply_mode_to_batch(baseline, key, mode="baspr")
...
def save_sparse_prune_result(result: SparsePruneResult, output_dir: str | Path, filename: str = "baspr_panel_64.json") -> Path:
